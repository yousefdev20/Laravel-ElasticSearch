...
final class Index
{
    ...
    public static function fromSource(ImportSource $source): Index
    {
        ...
        $settings = $settingsConfigKey;//config($settingsConfigKey, config('elasticsearch.indices.settings.default', $defaultSettings));
        $mappings = ['properties' => [
            'id' => [
                'type' => 'keyword',
            ],
        ],];//config($mappingsConfigKey, config('elasticsearch.indices.mappings.default'));

        return new static($name, $defaultSettings, $mappings);
    }
}
